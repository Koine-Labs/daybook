...
10. **Input is classified by intent, not modality (refined 2026-05-21).** Two categories at the L1 ingestion boundary: **Explicit** (user-initiated, communication-intended — voice/text, deliberate gestures → `chat_messages` / `user_actions`) and **Continuous** (ambient passive sensing — biometrics, prosody, vision, BCI, involuntary gestures → `sensor_readings`). Modalities cross-cut both (voice can be explicit speech or background mumbling; gestures can be deliberate or involuntary). Intent propagates through every layer of the pipeline with different interpretive weight. Output channels are voice (primary), future haptic, future visual indicator. See `docs/ARCHITECTURE.md §2.10` for full rule + lineage.
