...
13. **Regis is a controlled variable in his own predictive model (added 2026-05-21).** The decision layer eventually models how Regis's actions shape predicted future state — decisions become forward-looking treatment-effect estimation, not just retrospective scoring of "did interjecting help." Thompson contextual bandit is the seed; counterfactual reasoning ("if Regis does X vs Y, predicted state at t+1 = ?") is the destination. Purely reactive Regis architectures are inconsistent with the long-term direction. See `docs/ARCHITECTURE.md §2.13`.
